...
def train(args, model, train_loader, optimizer, criterion, epoch):
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        writer.add_scalar(
            "run/match", loss_match.item(), epoch * len(train_loader) + batch_idx
        )
        writer.add_scalar(
            "run/qa_test", loss_qa.item(), epoch * len(train_loader) + batch_idx
        )
        writer.add_scalar(
            "run/both", loss.item(), epoch * len(train_loader) + batch_idx
        )
...
def main():
    ...
    parser.add_argument(
        "--audio_dir",
        type=str,
        default="/home/guangyao_li/dataset/avqa-features/feats/vggish",
        help="audio dir",
    )
    # parser.add_argument(
    ...
    parser.add_argument(
        "--video_res14x14_dir",
        type=str,
        default="/home/guangyao_li/dataset/avqa-features/visual_14x14",
        help="res14x14 dir",
    )
    ...
    parser.add_argument(
        "--label_train",
        type=str,
        default="./data/json/avqa-train.json",
        help="train csv file",
    )
    parser.add_argument(
        "--label_val",
        type=str,
        default="./data/json/avqa-val.json",
        help="val csv file",
    )
    parser.add_argument(
        "--label_test",
        type=str,
        default="./data/json/avqa-test.json",
        help="test csv file",
    )
    parser.add_argument(
        "--batch-size",
    ...
    if args.mode == "train":
        ...
        ####### concat model
        pretrained_file = (
            "grounding_gen/models_grounding_gen/main_grounding_gen_best.pt"
        )
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if F >= best_F:
                best_F = F
                torch.save(
                    model.state_dict(), args.model_save_dir + args.checkpoint + ".pt"
                )
